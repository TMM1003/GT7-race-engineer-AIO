...
class AppController(QtCore.QObject):
    def __init__(self):
        ...
        self.window.sig_start_new_run_with_meta.connect(
            self._on_start_new_run_with_meta
        )
    ...
    def _tick(self) -> None:
        ...
        if (
            connected != self._dbg_last_connected
            or car_id != self._dbg_last_car_id
        ):
    ...
    def _build_connection_diagnostics(self, snap: dict) -> dict:
        ...
        mode = (
            "AUTO"
            if configured_ip in (None, "", discovery_sentinel)
            else "MANUAL"
        )
    ...
    def _telemetry_live(self) -> bool:
        # Strongest available signal: we are in-race and receiving valid
        # packets
        try:
    ...
    def _feature_spec_snapshot(self) -> dict:
        feats = list(
            self._research_features or list(self.research_cfg.features)
        )
        return {
    ...
    def _build_run_extra_meta(self) -> dict:
        spec = FeatureSpec(
            tuple(self._research_features or list(self.research_cfg.features))
        )
        ...
        return {
            ...
            "schema_hash": sh,
            "feature_spec": fss,
            "ui_visualization_rate_hz": (
                60 if getattr(self, "_viz_div", 6) == 1 else 10
            ),
            "buffer_samples": getattr(
                getattr(self.session, "_samples", None), "maxlen", None
            ),
    ...
    def _create_new_run(self) -> None:
        ...
        # Stamp extended run metadata right away (schema, features, buffer,
        # reference, etc.)
        try:
            self._patch_run_json(
                Path(self.run.run_dir), self._build_run_extra_meta()
            )
        except Exception:
            pass
    ...
    def _patch_manifest(self, run_dir: Path, entry: dict) -> None:
        ...
        data["updated_utc"] = time.strftime(
        ...
        try:
            export_lap_bundle(
                ...
                export_npz_if_available=bool(
                    self.research_cfg.export_npz_if_available
                ),
                export_json_always=bool(self.research_cfg.export_json_always),
                export_baselines=(
                    bool(self.research_cfg.export_delta_profile)
    ...
    def _on_export_dataset(self) -> None:
        ...
        try:
            ...
            lines.append(
                f"Rows: {report.rows_emitted}  (seen {
                    report.corners_seen
                }, skipped {report.corners_skipped})"
            )
            lines.append(
                f"Schema: v{report.schema_version}  hash {report.schema_hash}"
            )
            if report.reasons:
                lines.append("Skips:")
                for k, v in sorted(
    ...
    def _refresh_run_meta_ui(self) -> None:
        try:
            ...
            run_dir = (
                str(getattr(self.run, "run_dir", "")) if self.run else None
            )
    ...
    def _on_start_new_run_with_meta(self, meta: dict) -> None:
        ...
        if not track or not car:
            QtWidgets.QMessageBox.warning(
                ...
                "Missing metadata",
                (
                    "Track name and car name should be set before "
                    "starting a new run for research exports."
                ),
            )
            return
